...
class MKLDNNNode(object):
    '''An intermediate representation for MKLDNN operations.'''

    def __init__(self, op, *args, **kwargs):
        # A string corresponding to the MKLDNN operation
        self.op = op
        self.orig_op = op
...
class MaybeActivated(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        return MKLDNNNode(*args, **kwargs)


class MKLDNNMapper(IRNodeMapper):
    ...
    def map_relu(self, node):
        ...
        h_i = node.parents[0].output_shape[2]
        w_i = node.parents[0].output_shape[3]
        return MKLDNNNode('relu', c_i, w_i, h_i, 'triNNity::ACTIVATION_RELU')
    ...
    def map_pooling(self, node):
        ...
        kernel_params = self.get_kernel_params(node)
        return MKLDNNNode(pool_op, c_i, w_i, h_i, k_w, k_h, s_w, s_h, c_o, w_o, h_o)
    ...
    def map_softmax(self, node):
        return MKLDNNNode('softmax', node.parents[0].output_shape[1])
    ...
    def map_lrn(self, node):
        ...
        return MKLDNNNode('lrn', c_i, w_i, h_i, 'triNNity::layout::CHW', **kwargs)
    ...
    def map_concat(self, node):
        ...
        return MKLDNNNode('concat', c_i_0, c_i_1, w_i, h_i, 'triNNity::layout::CHW')

    def map_dropout(self, node):
        return MKLDNNNode('dropout', node.parameters.dropout_ratio)
    ...
    def map_eltwise(self, node):
        ...
        for x in node.output_shape:
            elt_count *= x
        try:
            return MKLDNNNode(operations[op_code], elt_count)
        except KeyError:
    ...
    def map_flatten(self, node):
        ...
        h_i = node.parents[0].output_shape[2]
        w_i = node.parents[0].output_shape[3]
        return MKLDNNNode('flatten', c_i, w_i, h_i)

    def commit(self, chains):
        return chains


class MKLDNNEmitter(object):
...
class MKLDNNTransformer(object):
